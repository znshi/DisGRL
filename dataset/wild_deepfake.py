...
class WildDeepfake(AbstractDataset):
    ...
    def __init__(self, cfg, seed=2022, transforms=None, transform=None, target_transform=None):
        ...
        self.root = cfg['root']#/home/public/dataset/wild_df
        self.num_train = cfg.get('num_image_train', None)#None
        self.num_test = cfg.get('num_image_test', None)#None
    ...
    def __get_images(self):
        if self.split == 'train':
            num = self.num_train
            split_real = 'real_train/'
            split_fake = 'fake_train/'
        elif self.split == 'test':
            num = self.num_test
            split_real = 'real_test/'
            split_fake = 'fake_test/'
        else:
            num = None
        real = torch.load(join(self.root, self.split,"real.pickle"))
        real_images = []
        for i in real:
            item = split_real + i
        ...
        fake = torch.load(join(self.root, self.split, "fake.pickle"))
        fake_images = []
        for i in fake:
            item = split_fake + i
