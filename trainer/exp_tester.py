...
class ExpTester(AbstractTrainer):
    def __init__(self, config, stage="Test"):
        ...
        self.logdir = 'visual/NTvisual_16692/'
    ...
    def test(self, display_images=False):
        ...
        for idx, test_data in test_generator:
            ...
            if self.gpu:
                ...
                in_I, Y = (I, Y)
            Y_pre,recon_x1,recon_x2,iea = self.model(in_I)
            '''
            plot_img = make_grid(in_I, padding=4, normalize=True, scale_each=True, pad_value=1)
            plot_recon1 = make_grid(recon_x1.detach().float(), padding=4, normalize=True, scale_each=True, pad_value=1)
            plot_recon2 = make_grid(recon_x2.detach().float(), padding=4, normalize=True, scale_each=True, pad_value=1)
            img = self.plot(plot_img)
            recon1 = self.plot(plot_recon1)
            recon2 = self.plot(plot_recon2)
            
            writer.add_figure("test/Img", img, idx)
            writer.add_figure("test/recon1", recon1, idx)
            writer.add_figure("test/recon2", recon2, idx)
            '''            
            for i in range(iea.shape[1]):
                writer.add_image(f'testiea/', iea[0][i].detach().cpu().unsqueeze(dim=0), idx)
            
            from  torchvision import utils as vutils
            savedir = '/home/public/visiual/ff++NT/'
            if not os.path.exists(savedir):
                os.makedirs(savedir)
            for i in range(recon_x1.shape[0]):
                vutils.save_image(recon_x1[i], '{}/{}_{}_recon1.png'.format(savedir,idx,i), normalize=True)
                vutils.save_image(recon_x2[i], '{}/{}_{}_recon2.png'.format(savedir,idx,i), normalize=True)
                vutils.save_image(recon_x1[i]-in_I[i], '{}/{}_{}_recon_cha1.png'.format(savedir,idx,i), normalize=True)
                vutils.save_image(recon_x2[i]-in_I[i], '{}/{}_{}_recon_cha2.png'.format(savedir,idx,i), normalize=True)
