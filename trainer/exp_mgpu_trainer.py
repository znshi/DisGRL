...
class ExpMultiGpuTrainer(AbstractTrainer):
    ...
    def _train_settings(self, model_cfg, data_cfg, config_cfg):
        ...
        self.resume = config_cfg.get("resume", False)#接续训练，在config文件中写一行resume：接续训练的模型地址
        ...
        try:
            ...
            for epoch_idx in range(start_epoch, self.num_epoch + 1):
                ...
                for batch_idx, train_data in train_generator:
                    ...
                    #print('math.ceil(self.num_steps / len(self.train_loader)):',math.ceil(self.num_steps / len(self.train_loader)))
    ...
    def validate(self, epoch, step, timer, writer):
        ...
        with torch.no_grad():
            ...
            for val_idx, val_data in val_generator:
                ...
                writer.add_figure("val/Img", img, step)
                writer.add_figure("val/recon1", recon1, step)
                writer.add_figure("val/recon2", recon2, step)
                        
                for i in range(iea.shape[1]):
                    writer.add_image(f'iea/', iea[0][i].detach().cpu().unsqueeze(dim=0), step)
                '''
                ...
                if val_idx == v_idx or val_idx == 1:
                    ...
                    # show images
                    images = I[:4]#N,3,224,224
                    images = torch.cat([images, *sample_recons], dim=0)#N*3,3,224,224
                    pred = Y_pre[:4]
                    gt = Y[:4]
